...
// Geçmiş mesajları ve yeni kullanıcı mesajını birleştiriyoruz
string context = string.IsNullOrEmpty(history) ? "" : history + "\n";
context += $"You: {userMessage}\n"; // Kullanıcı mesajı eklendi

// Cevabın kısa olmasını istediğimizi belirtiyoruz
string prompt = $"{context}Respond as a helpful English language tutor to this message. Please provide a concise and informative response, avoiding long replies. Limit the response length to a reasonable size.";

// Yanıtı almak için dil öğrenme servisini çağırıyoruz
