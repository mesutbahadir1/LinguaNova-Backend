    ...
    public class GeminiService
    {
        ...
        public async Task<string> GetLanguageLearningResponse(string prompt, string targetLanguage)
        {
            try
            {
                ...
                _logger.LogInformation($"Successful response content length: {responseString.Length}");
        ...
        // Method for general chatbot conversation with specialized instructions
        public async Task<string> GetChatbotResponse(string userMessage, string history)
        {
            // Combine history and new user message 
            string context = string.IsNullOrEmpty(history) ? "" : history + "\n";
            context += $"User: {userMessage}\n";

            // Create a prompt that specifies our expectations
            string prompt = $"{context}You're a friendly and engaging English tutor having a casual conversation with your student. Reply to the student's last message in a natural, conversational tone. Keep it concise, clear, and helpful—explain things simply when needed. Ask a follow-up question or make a friendly comment to keep the conversation going.";


            // Get response using the language learning service
            return await GetLanguageLearningResponse(prompt, "English");
        }

        // Method for grammar correction with specialized instructions
        public async Task<string> GetGrammarCorrection(string text)
        {
            return await GetLanguageLearningResponse(
                $"You are a grammar expert. Please correct any grammar mistakes in this text and explain the corrections clearly but concisely. Text to correct: {text}",
                "English");
        }

        // Method for vocabulary explanation with specialized instructions
        public async Task<string> GetVocabularyExplanation(string word)
        {
            return await GetLanguageLearningResponse(
                $"You are a vocabulary expert. For the word '{word}', please provide: 1) Definition, 2) Part of speech, 3) Example sentences, 4) Synonyms/antonyms if applicable, and 5) Any useful language learning tips related to this word. but make it concisely",
                "English");
        }
    }

    // Response classes for deserialization based on actual API response
